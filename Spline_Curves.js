...
//  Cardinal Blending Fuctions:

function C_0(u){
  var x = -tension*Math.pow(u,3) + 2*tension*Math.pow(u,2) - tension*u;
  return x;
}

function C_1(u){
  var x = (2-tension)*Math.pow(u,3) + (tension-3)*Math.pow(u,2) + 1;
  return x;
}

function C_2(u){
  var x = (tension-2)*Math.pow(u,3) + (3-2*tension)*Math.pow(u,2) + tension*u;
  return x;
}

function C_3(u){
  var x = tension*Math.pow(u,3) - tension*Math.pow(u,2);
  return x;
}


//  Interpolation via Cardinal Splines

function interpolate(pk_n1,pk,pk_p1,pk_p2)
{
  for(var u=0; u<=1; u+=uStep){

    var x = C_0(u)*pk_n1.x + C_1(u)*pk.x + C_2(u)*pk_p1.x + C_3(u)*pk_p2.x;
    var y = C_0(u)*pk_n1.y + C_1(u)*pk.y + C_2(u)*pk_p1.y + C_3(u)*pk_p2.y;
